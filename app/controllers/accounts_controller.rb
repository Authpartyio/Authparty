...
class AccountsController < ApplicationController
  ...
  def new
    ...
    if params[:redirect] != nil
      ...
      value = "?action=sign&message=" + URI.escape(@generated_message.to_s) + "&icon=" + url_encode(@provider.logo) + "&callback=" + url_encode(@callback)
    else
      @callback = URI.escape(ENV['BASE_API_URL'] + '/api/v1/authorize_login?modal_id=' + @modal_id)
      value = "?action=sign&message=" + URI.escape(@generated_message.to_s) + "&callback=" + url_encode(@callback)
    end
    @qr_data = value
  end
