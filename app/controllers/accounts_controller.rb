...
class AccountsController < ApplicationController
  ...
  def new
    ...
    if params[:redirect] != nil
      ...
      tapscan_value = "?action=sign&message=" + URI.escape(@generated_message.to_s) + "&icon=" + url_encode('http://authparty.io/Assets/authparty-icon-48x48.png') + "&callback=" + url_encode(@callback)
    end
    @qr_data = value
  ...
  def show
    ...
    # Check for Early Access Token (XAPEA)
    @early_access_token = false
    address_tokens = HTTParty.get('https://xchain.io/api/balances/' + @account.public_key)
    if address_tokens['error']
        @address_error = true
    else 
